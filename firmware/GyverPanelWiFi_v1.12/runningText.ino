...
String processMacrosInText(const String text) {  
  ...
  */
  ...
  while (!found && (attempt < sizeOfTextsArray)) {
    ...
    // -------------------------------------------------------------
    // {WSR} - выводить время рассвета
    // -------------------------------------------------------------
    idx = textLine.indexOf("{WSR}");
    if (idx >= 0) {
      textLine.replace("{WSR}", sunrise);
    }

    // -------------------------------------------------------------
    // {WSS} - выводить время заката
    // -------------------------------------------------------------
    idx = textLine.indexOf("{WSS}");
    if (idx >= 0) {
      textLine.replace("{WSS}", sunset);
    }
    #endif
