...
#define EEPROM_OK     0x3D       // Флаг, показывающий, что EEPROM инициализирована корректными данными 
...
#ifndef DEFAULT_IP
#define DEFAULT_IP {192, 168, 0, 254}            // Сетевой адрес устройства по умолчанию
#endif
...
#define EFFECT_LIST F("Часы,Свет,Снег,Куб,Радуга,Пейнтбол,Огонь,Matrix,Шарики,Метеоры,Конфетти,Переливы," \
                      "Дизайн,Контакт,Звезды,Штора,Цвета,Светляки,Палитра,Циклон,Хаос,Сияние,Тени," \
                      "Лабиринт,Змейка,Тетрис,Теннис,Flappy,Runner,Одеяло,Спектр,Синус,Узор,Дождь,Камин," \
                      "Стрелки,Погода,Жизнь,Узоры,Sinus,Попкорн,ДНК,Прыжки,Спирали,Ветер,Nexus,Синусоид,Питон,"\
                      "Cube,Quantum,Фeя,Kaпли,Ocциллятop,Kипeниe,Дpaжe,Boлны,Круг,Пикacco,Люмeньep,Шapы,Море,"\
                      "Дым,Пульс,Вода,Вихри,Кометы,Змей,Паук,Фонтан,Аврора,Часы2,Салют,Трек,Краски,Свеча,"\
                      "Рубик,Кудри,Лотос,Елка,Песок,Лента,Star,Лампа,Замок,Серпантин,Tixy,"\
                      "Утро,SD")
#else
#define EFFECT_LIST F("Часы,Свет,Снег,Куб,Радуга,Пейнтбол,Огонь,Matrix,Шарики,Метеоры,Конфетти,Переливы," \
                      "Дизайн,Контакт,Звезды,Штора,Цвета,Светляки,Палитра,Циклон,Хаос,Сияние,Тени," \
                      ...
                      "Cube,Quantum,Фeя,Kaпли,Ocциллятop,Kипeниe,Дpaжe,Boлны,Круг,Пикacco,Люмeньep,Шapы,Море,"\
                      "Дым,Пульс,Вода,Вихри,Кометы,Змей,Паук,Фонтан,Аврора,Часы2,Салют,Трек,Краски,Свеча,"\
                      "Рубик,Кудри,Лотос,Елка,Песок,Лента,Star,Лампа,Замок,Серпантин,Tixy,Утро")
#endif
#else
#if (USE_SD == 1)                      
#define EFFECT_LIST F("Часы,Свет,Снег,Куб,Радуга,Пейнтбол,Огонь,Matrix,Шарики,Метеоры,Конфетти,Переливы," \
...
#define MC_FILL_COLOR            1         // Свет
#define MC_SNOW                  2         // Снегопад
#define MC_BALL                  3         // Кубик
#define MC_RAINBOW               4         // Радуга
#define MC_PAINTBALL             5         // Пейнтбол
#define MC_FIRE                  6         // Огонь
#define MC_MATRIX                7         // Matrix
...
#define MC_HOURGLASS            79         // Песочные часы
...
#define MC_SERPENTINE           84         // Серпантин
#define MC_TIXY                 85         // TIXY

// Часы в фоторамке - размер 17x11 - для них нет картинки анимации - эффект анимация удален из прошивки
#ifdef NO_ANIMATION
#define MC_DAWN_ALARM           86         // Рассвет         
#define MC_SDCARD               87         // SD-карта

#if (USE_SD == 1)
#define MAX_EFFECT              88         // количество эффектов, определенных в прошивке
#else
#define MAX_EFFECT              87         // количество эффектов, определенных в прошивке
#endif
#else
#define MC_IMAGE                86         // Анимция
#define MC_DAWN_ALARM           87         // Рассвет
#define MC_SDCARD               88         // SD-карта

#if (USE_SD == 1)
#define MAX_EFFECT              89         // количество эффектов, определенных в прошивке
#else
#define MAX_EFFECT              88         // количество эффектов, определенных в прошивке
#endif
#endif
...
//Переменные для эффекта Водопад
uint8_t COOLINGNEW;    //интенсивность потока (?) рекомендуют менять от 20 или даже от 8 до 100
uint8_t SPARKINGNEW;   //длина струй (?)          рекомендуют менять от 50 до 200
