...
//
//Доработки от меня:
//Добавлены игры Flappy Bird и Runner из первой версии проекта Gyver BT Matrix с адаптацией под текущий проект (примечание: играбельны только на максимальной скорости).
//Добавлен красный фон на завершение всех игр (был только в "Змейке")
//Добавлен эффект "Синусоиды" из первой версии проекта Gyver BT Matrix с адаптацией под текущий проект (переписаны таймеры, т. к. изначально эффект очень жестко тормозил).
//Добавлен ползунок варианта на изменение количества синусиоид от 1 до 4.
//Раскомментировал строку (showTextNow && !fullTextFlag) в файле custom.h, чтобы эффект не менялся, пока не пробежит вся строка.
...
//Закомментировал строчку setManualModeTo(true) в a_main.h т. к. задолбало каждый раз ставить галку автопереключения режимов после выбора режима вручную.
//Добавил эффекты из прошивки gunner47_v2. Объединил имеющийся эффект PRIZMATA (Синусы) с одноименным эффектом из прошивки gunner47. Теперь их два в списке Вариант в параметрах эффекта.
//Некоторые эффекты объединял по смыслу и если они представляют собой вызов одной функции с разными параметрами, поэтому фактически добавленных эффектов стало еще больше.
//Сократил названия эффектов до максимально простых и коротких, поскольку беда пришла, откуда не ждали: уперлись в ограничение длины строки и больше в список эффектов ничего не добавить.
...
//Убрал ползунок варианта из добавленных эффектов, где он не нужен. Добавил ползунок варианта на изменение количества червячков в пределах 1..2*pWIDTH в эффекте NEXUS.
//Добавил эффект "Плазмолампа". Вычистил закомментированные строки в файле с эффектами. Хотел сделать последовательное воспроизведение всех вариантов в эффекте WAVES, но что-то пошло не так.
...
//Исправил зависание игр в игровом режиме, когда подключен SD-модуль (объявлен #define USE_SD 1). Добавил очистку экрана при вызове рисовалки и загрузки изображения.
//Добавил в бегущую строку макросы на IP-адрес, SSID и версию прошивки.
//Исправил работу эффектов, добавленных мной лично. Проблема была в том, что в них использовались значения HEIGHT и WIDTH из соответствующих define-ов. 
//Посему настройка параметров матрицы в приложении PanelSetup приводила к неверному отображению эффектов. Заменил HEIGHT и WIDTH на pHEIGHT и pWIDTH во всех эффектах.
//Переобъявил массивы, необходимые для работы эффектов из прошивки gunner47 как динамические. В то же время пришлось объявить трехмерный массив noise_3d в файле a_def_soft.h  
//и иницализировать его в функции setup(), т. к. вне функций возможно объявлять только статические массивы с константами. Поправил нахождение середины в эффекте Драже.
//В эффекте Prizmata (1 вариант) добавил случайность на направление синусов (вертикально/горизонтально). Навел порядок в в эффекте Sinusoid (распихал код по отдельным функциям),
//после чего нормально заработали все 8 вариантов. Также разбил на отдельные функции эффект "Волны". 
//В эффекте Пикассо добавил параметр Варианты. Теперь доступен список из 3 позиций, т. к. все 3 варианта разные и прикольные.
//Разобрался с эффектом Спирали. Теперь он нормально получает размеры матрицы из памяти, а не из DEFINE-ов в коде, как раньше, и выводится на всю матрицу вне зависимости от ее размеров.
//Добавил в качестве второго варианта к эффекту Nexus змеек, тем более, что эти два эффекта немного похожи. Также добавил эффект "Аврора". Странно, что не сделал это раньше.
//Добавил Часы с циферблатом, Салют, Firework (Салют и Firework объединены в один эффект), Swirl, Watercolor, FeatherCandle, Кубик Рубика от vvip-68, Color Frizzles, Lotus Flower и Елку.
//Произвел некоторые мелкие доработки. Включил ползунок скорости у камина, поскольку он был слишком быстрый. Отключил ненужные ползунки. Добавил в узоры Гвардейскую ленту на зеленом фоне.
//Переделал пламя свечи. Для низких матриц оно было странное, теперь больше напоминает настоящее (но все равно стремное). Уменьшил предел высоты матрицы для анимированного пламени до 13.
//Проверено на матрице с ограничением 13 пикселей по высоте - все OK. Свеча немного перерисована. Цветов стало меньше, зато нормально отрисовывается на малых яркостях.
//Часы с циферблатом теперь всегда занимают квадратную область с размерами, соответствующими короткой стороне матрицы.
//Добавлены ползунки варианта на изменение количества мячиков и попкорнин в соответствующих эффектах.
//Исправлена ситуация с палитрой в варианте 2 эффекта Prizmata. Изменен алгоритм на выбор направления эффекта Prizmata в зависимости от размеров матрицы.
//Теперь если матрица широкая, эффект работает горизонтально, на высоких матрицах включается вертикально, а на квадратных матрицах может запуститься в произвольном направлении. 
//Исправлены рандомы в ряде добавленных эффектов. Теперь везде, где есть список вариантов, режим "Случайный" работает нормально.
//Убран ночной режим в часах с циферблатом, т. к. он тупой, а работать матрица будет в том числе ночью.  
//Шумовые эффекты объединены в один, добавлены эффекты Дизайн, Контакт, Звёзды, Штора. Перенесена часть правок по поводу бегущей строки из оригинальной прошивки. 
//Изменен алгоритм на выбор направления эффекта ДНК в зависимости от размеров матрицы (см. алгоритм работы Prizmata). В эффекте "Пульс" теперь 8 доступных вариантов.
//Добавлены еще 5 эффектов: Песок, Лента, Stars, Лампа, Пузыри, эффект Вышиванка переименован в Узор, Палитра - в Одеяло, Притяжение - в Quantum.
//Итого 87/86 эффектов с/без SD. С учетом всех доступных вариантов имеем свыше 160 эффектов без учета анимаций для эффекта SD, записанных на флешку.
//Доработку прошивки можно считать финальной, потому что теперь уж точно все, сокращать названия эффектов дальше некуда.
//P.S. Если какие-то из эффектов, добавленных мной, работают криво, просьба сообщать об этом и указывать рамеры/направление/точку подключения матрицы.
...
#define FIRMWARE_VER F("WiFiPanel v.1.12.2023.02.18") //переходим на обозначение версий по дате
...
void setup() {
  ...
  //Инициализируем переменные для эффекта Спирали
  spirotheta1 = 0;
  spirotheta2 = 0;
  ...
  //массив noise_3d, необходимый для работы эффектов из прошивки gunner.
  //Изврат, но щито поделать. Иначе не будет нормально работать, если менять размеры матрицы налету через приложение PanelSetup.
  //В effects массив объявляется только с константами, поэтому пришлось бы каждый раз перепрошивать контроллер, жестко забивая размеры матрицы в #define WIDTH и #define HEIGHT
