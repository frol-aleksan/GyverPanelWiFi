...
//Переделал пламя свечи. Для маленьких матриц оно было странное, теперь больше напоминает настоящее. Уменьшил предел высоты матрицы для анимированного пламени до 13.
//На матрицах высотой 13 пикселей все OK - проверено. Перерисовал свечу. Цветов в ней стало меньше, зато нормально отрисовывается на малых яркостях.
//Часы с циферблатом теперь всегда занимают квадратную область с размерами, соответствующими короткой стороне матрицы.
//Добавил ползунки варианта на изменение количества мячиков и попкорнин в соответствующих эффектах.
//Исправил ситуацию с палитрой и добавил вертикальное направление в варианте 2 эффекта Prizmata. Изменил алгоритм на выбор направления синусов в зависимости от размеров матрицы.
//Теперь если матрица широкая, синусы работают горизонтально, на высоких матрицах эффект включается вертикально, а на квадратных матрицах может запуститься в произвольном направлении. 
//Пока доработку прошивки можно считать финальной, т. к. теперь уже точно все. Добавлять уже больше нечего, неоткуда и некуда. Достигнут предел по длине списка эффектов до последней буквы.
//Итого имеем 86/85 эффектов с/без SD.
//
// -------------------------------------------------------------------------------------------------------

// ************************ WIFI ПАНЕЛЬ *************************

#define FIRMWARE_VER F("WiFiPanel v.1.12.2022.11.12") //переходим на обозначение версий по дате
