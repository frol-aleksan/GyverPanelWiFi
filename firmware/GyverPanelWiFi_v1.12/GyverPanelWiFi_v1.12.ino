...
//В эффекте Prizmata (1 вариант) добавил случайность на направление синусов (вертикально/горизонтально). Навел порядок в в эффектах Sinusoid и Волны (распихал код по отдельным функциям).
//В эффекте Пикассо добавил параметр Варианты, теперь доступен список из 3 позиций, т. к. все 3 варианта разные и прикольные.
//Разобрался с эффектом Спирали. Теперь он нормально получает динамически задаваемые размеры матрицы из памяти.
...
#define FIRMWARE_VER F("WiFiPanel v.1.12.2022.10.15") //переходим на обозначение версий по дате
