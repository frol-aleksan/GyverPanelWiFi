...
//
//Доработки от меня:
//Добавлены игры Flappy Bird и Runner из первой версии проекта Gyver BT Matrix с адаптацией под текущий проект
//Примечание: играбельны только на максимальной скорости.
//Добавлен красный фон на завершение всех игр (был только в "Змейке")
//Добавлен эффект "Синусоиды" из первой версии проекта Gyver BT Matrix с частичной адаптацией под текущий проект
//Можно задать количество синусиоид от 1 до 4 (лучше всего ставить 2-3)
//Раскомментировал строку (showTextNow && !fullTextFlag) в файле custom.h, чтобы эффект не менялся, пока не пробежит вся строка
//Максимальный размер кубика в соответствующем эффекте увеличен до 5*5
//Исправлен глюк с яркостью при переходе к предыдущему эффекту
//Исправлен глюк с демо-режимом игр после их запуска в игровом режиме
//Закомментил строчку setManualModeTo(true): в a_main.h т. к. задолбало каждый раз ставить галку автопереключения режимов после выбора режима вручную.
//Добавил тонну эффектов из прошивки gunner47_v2, теперь их стало просто д...я, а именно 75, если подключить SD-карту, или 74 без нее.
//Список вырос на 28 позиций, объединил имеющийся эффект PRIZMATA с аналогичным из gunner47. Теперь их два в списке Вариант в параметрах эффекта Синусы.
//Также некоторые добавленные эффекты объединял либо по смыслу, либо где они представляют собой вызов одной функции с разными параметрами.
//Сократил названия некоторых эффектов до максимально простых и коротких, т. к. беда пришла, откуда не ждали: уперлись в ограничение длины строки.
//Поменял порядок эффектов, чтобы анимация и SD-карта были в самом конце (гораздо меньше строк в списке эффектов и удобнее их нумеровать).
//Убрал ползунок варианта из добавленных эффектов, где он не нужен. Добавил ползунок варианта на изменение количества червячков 1..2*WIDTH в NEXUS.
//Добавил эффект "Плазмолампа". Вычистил закомментированный мусор. Пока доработку прошивки можно считать финальной.
//
// -------------------------------------------------------------------------------------------------------
