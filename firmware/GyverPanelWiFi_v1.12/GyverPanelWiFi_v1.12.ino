...
//Добавил эффекты из прошивки gunner47_v2. Список вырос на 28 позиций. Теперь эффектов стало просто д...я, а именно 75, если подключить SD-карту, или 74 без нее.
//Объединил имеющийся эффект PRIZMATA (Синусы) с одноименным эффектом из прошивки gunner47. Теперь их два в списке Вариант в параметрах эффекта.
...
//Итого список вырос на 29 пунктов и всего стало 76/75 эффектов с/без SD.
...
//Разобрался с эффектом Спирали. Теперь он нормально получает размеры матрицы из памяти, а не из DEFINE-ов в коде, как раньше.
...
//Пока доработку прошивки можно считать финальной, т. к. добавлять уже больше нечего, неоткуда и некуда (достигнут предел по длине списка эффектов). Итого имеем 77/76 эффектов с/без SD.
//
// -------------------------------------------------------------------------------------------------------

// ************************ WIFI ПАНЕЛЬ *************************

#define FIRMWARE_VER F("WiFiPanel v.1.12.2022.10.22") //переходим на обозначение версий по дате
...
void setup() {
  ...
  //Инициализируем трехмерный динамический массив noise_3d, необходимый для работы эффектов из прошивки gunner.
