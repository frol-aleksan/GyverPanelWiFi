...
//Также разбит на отдельные функции код эффекта Волны. 
...
//Добавлены эффекты Часы с циферблатом (эффект Часы2), Салют (2 варианта), Swirl, Watercolor, Свеча, Кубик Рубика от vvip-68, Color Frizzles, Lotus Flower и Елка.
...
//Шумовые эффекты с префиксом MC_NOISE_ (Облака, Лава, Шумящий лес и вот это все) объединены в эффект Переливы со списком выбора вариантов.
...
//К эффекту Салют добавлен третий вариант.
...
//К эффекту Водопад вместо варианта 4в1 прикручены палитры, добавлены ползунки параметра 1 (длина струй) и контраста (применен как параметр 2; количество струй).
...
//Взамен эффекта Кометы добавлен эффект Стая.
//К эффекту Светлячки вторым и третьим вариантом добавлен эффект Мотыльки с крылышками и без;
//Добавлен эффект Spin.
//Перенесены правки по поводу сломанной яндекс-погоды.

//Полез глубоко в код:
//Вычищен мусор в эффектах: Капли, Осциллятор;
//Убраны параметры modes[currentMode].Speed и modes[currentMode].Scale там, где они были, и добавлен заменяющий их функционал для доступных ползунков настроек:
//Питон (ползунок контраста - вихри);
//Паук (ползунок параметра 1 - количество линий);
//TIXY (ползунок параметра 1 - цвет);
//Синусоид (ползунок параметра 1 - размеры);
//Фея (Галка Поведение, ползунок параметра 1 - шлейф);
//Драже (ползунок параметра 1 - высота горки);
//Пикассо (ползунок параметра 1 - количество объектов);
//Дым (ползунок параметра 1 - цвет; при значении более 110 включается режим Разноцветный);
//Змей (ползунок параметра 1 - сдвиг радуги; при значении 0 - динамическая радуга);
//Фонтан (ползунок параметра 1 - количество объектов, ползунок контраста - цвет; 255 - режим Разноцветный);
//Аврора (ползунок контраста влияет на скорость, ползунок параметра 1 - цвет; при значении более 110 включается режим Разноцветный);
//Часы2 (ползунок контраста - цвет часовой стрелки, ползунок параметра 1 - цвет циферблата и минутной стрелки);
//Ветер (ползунок контраста - влияет на скорость, ползунок параметра 1 - количество объектов);
//Попкорн (ползунок контраста - влияет на скорость, ползунок параметра 1 - количество объектов);
//Люменьер (ползунок параметра 1 - длина хвоста);
//Круг (ползунок параметра 1 - влияет на скорость);
//ДНК (ползунок параметра 1 - влияет на скорость и размытие);
//Пульс (ползунок параметра 1 - влияет на цвет);
//Дизайн (ползунок параметра 1 - размер плиток);
//Песок (ползунок параметра 1 - влияет на цвет);
//Серпантин (ползунок параметра 1 - влияет на скорость и размытие);
//Притяжение (ползунок параметра 1 - количество объектов);
//Кубик Рубика 2D (ползунок контраста и ползунок параметра 1 - размер плиток; можно задавать прямоугольные плитки);
//Синусы (ползунок параметра 1 влияет на размытие 2 варианта эффекта, палитра теперь выбирается произвольно);
//Замок (scale заменено на значение 40; почему так - не помню. Но раз надо, значит надо);
//Контакт (ползунок контраста влияет на скорость, ползунок параметра 1 - цвет);
//Прыжки (ползунок контраста - размытие/шлейф от частиц, ползунок параметра 1 - количество частиц);
//Кипение (ползунок параметра 1 - интенсивность);
//Шары (ползунок контраста вроден как влияет на скорость);
//Капли (ползунок контраста - количество объектов, ползунок параметра 1 - цвет);
//Краски (оба ползунка что-то делают).

//Исправлены обнаруженные старые косяки и сделаны прочие доработки в эффектах:
//Пикассо - исправлен выбор варианта из списка;
//Водопад - значение параметра 1 инвертировано, оставлена одна переменная sNEW для всех эффектов с ползунком контраста в качестве параметра;
//Люменьер - добавлены варианты Радуга и Случайные цвета, ползунок параметра 1 - длина хвоста;
//Круг - добавлен вариант Радуга, ползунок параметра 1 - влияет на скорость;
//Прыжки (вариант Мячики) - добавлено размытие (как в варианте Прыгуны);
//Лаволампа - исправлен выбор палитры в варианте 1 и подправлены параметры, чтобы оба варианта работали на адекватной скорости;
//Свеча - в качестве временной меры добавлен сдвиг свечи с помощью ползунка параметра 1, чтобы она не была "распилена" на два окна.

//TODO:
//В перспективе попробовать добавить сегменты на ряд эффектов (Свеча, Песок и т. п.)
//Разобраться с #define trackingOBJECT_MAX_COUNT и заменить на нормальные переменные, где он еще остался
//Прикрутить эффекты со звуком, если получится

//Итого в деморежиме доступно 90/89 эффектов с/без SD. Со всеми вариантами имеем свыше 160 эффектов, если не считать анимации для эффекта SD.
// -------------------------------------------------------------------------------------------------------

// ************************ WIFI ПАНЕЛЬ *************************

#define FIRMWARE_VER F("WiFiPanel v.1.12.2024.12.25") //переходим на обозначение версий по дате
