...
//Некоторые эффекты объединял либо по смыслу, либо если они представляют собой вызов одной функции с разными параметрами, поэтому фактически добавленных эффектов стало еще больше.
...
//Убрал ползунок варианта из добавленных эффектов, где он не нужен. Добавил ползунок варианта на изменение количества червячков в пределах 1..2*pWIDTH (ранее - 2*WIDTH) в эффекте NEXUS.
//Добавил эффект "Плазмолампа". Вычистил закомментированный мусор в файле с эффектами. Хотел сделать последовательное воспроизведение всех вариантов в эффекте WAVES, но что-то пошло не так.
...
//Добавил эффекты Часы с циферблатом, Салют, Firework (объединены в один эффект), Swirl, Watercolor, FeatherCandle, Кубик Рубика от vvip, Color Frizzles, Lotus Flower и ChristmasTree.
...
#define FIRMWARE_VER F("WiFiPanel v.1.12.2022.11.05") //переходим на обозначение версий по дате
