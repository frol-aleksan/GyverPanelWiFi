...
#### Эффекты:
 ...
 - Звездопад
 - Конфетти
 - Шумовые эффекты с разными цветовыми палитрами (9 вариантов)
 - Дизайн
 - Контакт
 - Звезды
 - Штора
 ...
 - Демо-версия игры Runner - автоигра без возможности управления
 - Демо-версия игры Flappy Bird - автоигра без возможности управления
 - Палитра (лоскутное одеяло) 
 - Спектр (имитация графического индикатора спектра звукового сигнала). 
 - Движущийся синус (2 варианта)
 - Вышиванка
 - Дождь
 - Камин
 - Стрелки (5 вариантов)
 - Погода (слайдшоу или отображение текущих погодных условия)
 - Жизнь - автоигра на развитие популяции без возможности управления
 - Фоновые узоры (нотки, сердечки, снежинки, зигзаги и т.п., всего 34)
 - Синусоиды с произвольно генерируемыми параметами (фаза, амплитуда, цвет)
 - Попкрон
 - ДНК
 - Прыжки (2 варианта)
 - Cпиpaли
...
Данный подход позволяет разбирать матрицу для удобства хранения и транспортировки и расширять ее размеры по горизонтали путем последовательного соединения нескольких матриц, (учитывая необходимость подведения питания к точкам подключения каждой матрицы).
Опционально секции матрицы можно просто соединить последовательно между собой в одну линейную гирлянду.

Стоит иметь ввиду, что максимальное расстояние от контроллера до точки подключения, при котором матрица работает стабильно, составляет около 3 метров. Далее матрица начинает «шуметь» (поверх выводимых эффектов наблюдается сильное мерцание), а если это расстояние превышает 3.5-4 метра - в произвольном порядке мигают первые 15-20 модулей, а остальная часть матрицы вообще не горит. Поэтому если наблюдаются подобные артефакты, в первую очередь укорачиваем провод от контроллера до точки подключения.
...
На мой взгляд, самая дешевая на Алиэкспрессе, особенно если брать сразу много. Стоит иметь ввиду, что закупать гирлянды лучше сразу на весь проект, потому что от партии к партии яркость/цветовая температура может отличаться. За все время с 2020 по 2022 год заказал у этого продавца 23 гирлянды, из них оказался поврежденный всего один светодиод.
- Матрица 16*16 с контроллером: https://aliexpress.ru/item/1005003698849342.html
Пользуюсь для отладки прошивки, контроллер применил вместе с оставшимся огрызком гирлянды после сборки всех матриц. Просто матрицу можно найти дешевле, но вместе с контроллером это предложение остается более выгодным.
